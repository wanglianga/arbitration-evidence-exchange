...
5. **交换批次提交冻结**：一旦提交交换批次，该批次下所有证据的文件哈希、页码、排序、提交时间自动冻结，后续无法修改或新增，如需调整必须通过补充材料流程
6. **证据版本历史**：当事人每次修改证据信息（标题、描述、目录、页码等）系统均自动保留草稿版本快照，记录变更原因和修改人，可随时回溯历史版本
7. 已撤回的证据无法被质证或庭审引用，但保留历史记录可审计
8. **超期证据审核**：过了举证期限提交的证据不能直接混入正式目录，必须进入秘书-仲裁员二级审核流程，记录迟交理由、对方是否同意质证、仲裁员处理意见和是否影响开庭日期
